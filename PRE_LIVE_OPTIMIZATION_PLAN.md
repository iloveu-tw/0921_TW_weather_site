...
| P1-01 | P1 | 建立正確的縣市資料欄位與篩選 | 進行中 | CWA 欄位完整率盤點完成；依使用者指示暫停，尚未修改 Schema |
...
- 尚未修改 Neon Schema、同步資料、TypeScript 型別或前端篩選；目前可安全由此處恢復。
- 恢復後下一步：加入 `county`／`town` 欄位與解析，執行 876 筆原子同步，再驗證縣市統計與前端篩選。
