...
|---|:---:|---|:---:|---|
| P0-01 | P0 | 輪替憑證並移除版本控制中的敏感資訊 | 進行中 | 新 Key 已完成單筆 API 驗證；待舊 Key 停用與 Git 歷史清理 |
| P0-02 | P0 | 決定並建立正式環境資料架構 | 待辦 | — |
...
- 2026-09-21：確認 Git 歷史仍有 5 個 commits 含舊值，因此尚未完成。
- 2026-09-21：新 Key 已存入受 `.gitignore` 保護的本機 `.env`，並以 CWA 單筆唯讀 API 查詢驗證成功。
- 待使用者確認舊 Key 已於 CWA 後台停用。
- 待取得明確批准後清理 Git 歷史並安全更新遠端。
